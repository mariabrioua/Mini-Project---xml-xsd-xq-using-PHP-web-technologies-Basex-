...
if ($xml) {
    ...
    foreach ($xml->membres->membre as $m) {
        $id = (string)$m['id'];
        $membres[$id] = (string)$m->prenom . ' ' . (string)$m->nom;
        $membreCategories[$id] = (string)$m['categorieRef'];
    }

    foreach ($xml->concours->concours as $c) {
        $cid = (string)$c['id'];
        $concoursList[] = $c;
        $concoursCategories[$cid] = (string)$c['categorieRef'];
    }
}
...
// Handle inscription
// Important: a member can participate only in a competition of his/her own category
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'inscrire') {
    ...
    if (!$concoursId || !$membreId) {
    ...
    } elseif (!isset($concoursCategories[$concoursId])) {
        $error = "Competition not found.";
    } elseif (!isset($membreCategories[$membreId])) {
        $error = "Member not found.";
    } elseif ($concoursCategories[$concoursId] !== $membreCategories[$membreId]) {
        $error = "This member cannot be registered: the member category does not match the competition category.";
        ...
        foreach ($xml->concours->concours as $c) {
            if ((string)$c['id'] === $concoursId) {
                ...
                if ($alreadyRegistered) {
                    $error = "This member is already registered in this competition.";
                    $found = true;
                    break;
                }
        ...
        if ($found && !$alreadyRegistered && $error === '') {
            $dom = dom_import_simplexml($xml)->ownerDocument;
            $dom->formatOutput = true;
            $dom->save($xml_path);
            $message = "Registration saved successfully!";

            // Reload XML after saving
            $xml                 = loadXML($xml_path);
            $categories          = [];
            $membres             = [];
            $membreCategories    = [];
      ...
      <div class="form-row">
        <div class="form-group">
          ...
          <select name="concours" id="concours" class="select-input" required>
            ...
            <?php foreach ($concoursList as $c): ?>
              <option value="<?= htmlspecialchars((string)$c['id']) ?>"
                      data-category="<?= htmlspecialchars((string)$c['categorieRef']) ?>">
                <?= htmlspecialchars((string)$c->titre) ?>
              </option>
            <?php endforeach; ?>
          </select>
          ...
          <select name="membre" id="membre" class="select-input" required>
            ...
            ?>
              <option value="<?= htmlspecialchars($id) ?>"
                      data-category="<?= htmlspecialchars($catRef) ?>"
                      hidden>
                <?= htmlspecialchars($nom) ?>
              </option>
            <?php endforeach; ?>
          </select>
          <small class="field-help">Only members from the selected competition category are displayed.</small>
        </div>
      </div>
    ...
    <form method="POST" action="index.php#xquery-section" class="xquery-form">
      ...
      <div class="xquery-editor-wrap">
        <textarea name="xquery" id="xquery-input" class="xquery-textarea"
                  placeholder='e.g. for $m in db:get("club")//membre return $m/nom/text()'
...
// Filter members according to the selected competition category
const concoursSelect = document.getElementById('concours');
const membreSelect = document.getElementById('membre');

function filterMembersByCategory() {
  if (!concoursSelect || !membreSelect) return;

  const selectedOption = concoursSelect.options[concoursSelect.selectedIndex];
  const selectedCategory = selectedOption ? selectedOption.getAttribute('data-category') : '';
  ...
  for (const option of membreSelect.options) {
    if (option.value === '') {
      option.hidden = false;
      option.textContent = selectedCategory ? 'Select...' : 'Select a competition first...';
      continue;
    }

    const memberCategory = option.getAttribute('data-category');
    const show = selectedCategory && memberCategory === selectedCategory;
...
if (concoursSelect && membreSelect) {
  concoursSelect.addEventListener('change', filterMembersByCategory);
